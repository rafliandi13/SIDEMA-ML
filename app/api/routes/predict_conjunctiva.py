"""Conjunctiva prediction route."""

from fastapi import APIRouter, Depends, File, Form, UploadFile

from app.api.deps import (
    get_app_settings,
    get_conjunctiva_inference_service,
...
@router.post("/conjunctiva", response_model=SuccessResponse)
async def predict_conjunctiva(
    ...
    inference_service: ConjunctivaInferenceService = Depends(get_conjunctiva_inference_service),
    repository: PredictionRepository = Depends(get_prediction_repository),
):
    """Run conjunctiva prediction pipeline and persist metadata."""
    ...
    file_key = storage_service.build_object_key(
        user_id=user_id,
        method=PredictionMethod.CONJUNCTIVA.value,
    ...
    return success_response(data=payload.model_dump(mode="json"), message="Conjunctiva prediction completed.")
